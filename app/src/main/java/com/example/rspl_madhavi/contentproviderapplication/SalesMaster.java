package com.example.rspl_madhavi.contentproviderapplication;

/**
 * Created by rspl-aman on 11/5/17.
 */

public class SalesMaster {
    public SalesMaster() {
    }

    public String getTRI_ID() {
        return TRI_ID;
    }

    public void setTRI_ID(String TRI_ID) {
        this.TRI_ID = TRI_ID;
    }

    public String getSTORE_ID() {
        return STORE_ID;
    }

    public void setSTORE_ID(String STORE_ID) {
        this.STORE_ID = STORE_ID;
    }

    public String getGRAND_TOTAL() {
        return GRAND_TOTAL;
    }

    public void setGRAND_TOTAL(String GRAND_TOTAL) {
        this.GRAND_TOTAL = GRAND_TOTAL;
    }

    public String getBUSINESS_DATE() {
        return BUSINESS_DATE;
    }

    public void setBUSINESS_DATE(String BUSINESS_DATE) {
        this.BUSINESS_DATE = BUSINESS_DATE;
    }

    public String getSALE_DATE() {
        return SALE_DATE;
    }

    public void setSALE_DATE(String SALE_DATE) {
        this.SALE_DATE = SALE_DATE;
    }

    public String getSALE_TIME() {
        return SALE_TIME;
    }

    public void setSALE_TIME(String SALE_TIME) {
        this.SALE_TIME = SALE_TIME;
    }

    public String getCARD_NO() {
        return CARD_NO;
    }

    public void setCARD_NO(String CARD_NO) {
        this.CARD_NO = CARD_NO;
    }

    public String getTOTAL_POINTS() {
        return TOTAL_POINTS;
    }

    public void setTOTAL_POINTS(String TOTAL_POINTS) {
        this.TOTAL_POINTS = TOTAL_POINTS;
    }

    public String getSCHEME_POINTS() {
        return SCHEME_POINTS;
    }

    public void setSCHEME_POINTS(String SCHEME_POINTS) {
        this.SCHEME_POINTS = SCHEME_POINTS;
    }

    public String getFLAG() {
        return FLAG;
    }

    public void setFLAG(String FLAG) {
        this.FLAG = FLAG;
    }

    public String getCREATED_BY() {
        return CREATED_BY;
    }

    public void setCREATED_BY(String CREATED_BY) {
        this.CREATED_BY = CREATED_BY;
    }

    public String getCREATED_ON() {
        return CREATED_ON;
    }

    public void setCREATED_ON(String CREATED_ON) {
        this.CREATED_ON = CREATED_ON;
    }

    public String getCUST_ID() {
        return CUST_ID;
    }

    public void setCUST_ID(String CUST_ID) {
        this.CUST_ID = CUST_ID;
    }

    public String getS_FLAG() {
        return S_FLAG;
    }

    public void setS_FLAG(String s_FLAG) {
        S_FLAG = s_FLAG;
    }

    public String getPOS_USER() {
        return POS_USER;
    }

    public void setPOS_USER(String POS_USER) {
        this.POS_USER = POS_USER;
    }

    public String getM_FLAG() {
        return M_FLAG;
    }

    public void setM_FLAG(String m_FLAG) {
        M_FLAG = m_FLAG;
    }

    public String getEX_TRI_ID() {
        return EX_TRI_ID;
    }

    public void setEX_TRI_ID(String EX_TRI_ID) {
        this.EX_TRI_ID = EX_TRI_ID;
    }

    public String getORDER_TYPE() {
        return ORDER_TYPE;
    }

    public void setORDER_TYPE(String ORDER_TYPE) {
        this.ORDER_TYPE = ORDER_TYPE;
    }

    public String getPICK_UP_LOCATION() {
        return PICK_UP_LOCATION;
    }

    public void setPICK_UP_LOCATION(String PICK_UP_LOCATION) {
        this.PICK_UP_LOCATION = PICK_UP_LOCATION;
    }

    public String getDISCOUNT_PERCENT() {
        return DISCOUNT_PERCENT;
    }

    public void setDISCOUNT_PERCENT(String DISCOUNT_PERCENT) {
        this.DISCOUNT_PERCENT = DISCOUNT_PERCENT;
    }

    public String getLINE_ITEM_DISC() {
        return LINE_ITEM_DISC;
    }

    public void setLINE_ITEM_DISC(String LINE_ITEM_DISC) {
        this.LINE_ITEM_DISC = LINE_ITEM_DISC;
    }

    public String getLINE_DISC() {
        return LINE_DISC;
    }

    public void setLINE_DISC(String LINE_DISC) {
        this.LINE_DISC = LINE_DISC;
    }

    public String getCUST_NAME() {
        return CUST_NAME;
    }

    public void setCUST_NAME(String CUST_NAME) {
        this.CUST_NAME = CUST_NAME;
    }

    public String getCUST_CNCT() {
        return CUST_CNCT;
    }

    public void setCUST_CNCT(String CUST_CNCT) {
        this.CUST_CNCT = CUST_CNCT;
    }

    public String getDOCT_NAME() {
        return DOCT_NAME;
    }

    public void setDOCT_NAME(String DOCT_NAME) {
        this.DOCT_NAME = DOCT_NAME;
    }

    public String getDOCT_SPEC() {
        return DOCT_SPEC;
    }

    public void setDOCT_SPEC(String DOCT_SPEC) {
        this.DOCT_SPEC = DOCT_SPEC;
    }

    private String TRI_ID = "TRI_ID";
    private String STORE_ID="STORE_ID";
    private String GRAND_TOTAL= "GRAND_TOTAL";
    private String BUSINESS_DATE= "BUSINESS_DATE";
    private String SALE_DATE=  "SALE_DATE";
    private String SALE_TIME= "SALE_TIME";
    private String CARD_NO = "CARD_NO";
    private String TOTAL_POINTS =  "TOTAL_POINTS";
    private String SCHEME_POINTS= "SCHEME_POINTS";
    private String FLAG="FLAG";
    private String CREATED_BY = "CREATED_BY";
    private String CREATED_ON = "CREATED_ON";

    private String CUST_ID=  "CUST_ID";
    private String S_FLAG= "S_FLAG";
    private String POS_USER="POS_USER";
    private String M_FLAG ="M_FLAG";
    private String EX_TRI_ID= "EX_TRI_ID";
    private String ORDER_TYPE="ORDER_TYPE";
    private String PICK_UP_LOCATION =  "PICK_UP_LOCATION";
    private String DISCOUNT_PERCENT=  "DISCOUNT_PERCENT";
    private String LINE_ITEM_DISC=  "LINE_ITEM_DISC";
    private String LINE_DISC= "LINE_DISC";
    private String CUST_NAME= "CUST_NAME";
    private String CUST_CNCT= "CUST_CNCT";
    private String DOCT_NAME= "DOCT_NAME";
    private String DOCT_SPEC= "DOCT_SPEC";

}
